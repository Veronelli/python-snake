...
class PathSpec(object):
    ...
    def check_files(
        ...
        *separators* (:class:`~collections.abc.Collection` of :class:`str`; or
        ...
        if not _is_iterable(
            ...
            raise TypeError(
                f"files:{files!r} is not an iterable."
            )

        use_patterns = _filter_check_patterns(
            self.patterns
        )
        for (
            orig_file
    ...
    def from_lines(
        ...
        *lines* (:class:`~collections.abc.Iterable`) yields each uncompiled pattern
        ...
        if isinstance(
            ...
            pattern_factory = util.lookup_pattern(
                pattern_factory
            )
        ...
        if not _is_iterable(
            ...
            raise TypeError(
                f"lines:{lines!r} is not an iterable."
            )
    ...
    def match_entries(
        ...
        *negate* (:class:`bool` or :data:`None`) is whether to negate the match
        ...
        if not _is_iterable(
            ...
            raise TypeError(
                f"entries:{entries!r} is not an iterable."
            )
    ...
    _match_file = staticmethod(
        util.check_match_file
    )
    """
