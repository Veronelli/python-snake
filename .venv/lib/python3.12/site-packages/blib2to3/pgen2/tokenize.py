...
Name = r"[^\s#\(\)\[\]\{\}+\-*/!@$%^&=|;:'\",\.<>/?`~\\]+"  # this is invalid but it's fine because Name comes after Number in all groups
...
FstringMiddleAfterColon = (
    group(
        Whitespace + r".*?"
    )
    + group("{", "}")
)
fstring_middle_after_colon = re.compile(
    FstringMiddleAfterColon
)
...
singleprog = re.compile(
    Single
)
singleprog_plus_lbrace = (
    re.compile(
        group(
            SingleLbrace,
            Single,
...
doubleprog = re.compile(
    Double
)
doubleprog_plus_lbrace = (
    re.compile(
        group(
            DoubleLbrace,
            Double,
...
single3prog = re.compile(
    Single3
)
single3prog_plus_lbrace = (
    re.compile(
        group(
            Single3Lbrace,
            Single3,
...
double3prog = re.compile(
    Double3
)
double3prog_plus_lbrace = (
    re.compile(
        group(
            Double3Lbrace,
            Double3,
...
endprogs: Final = {
    ...
    **{
        f"{prefix}'": singleprog_plus_lbrace
        for prefix in _fstring_prefixes
    },
    **{
        f'{prefix}"': doubleprog_plus_lbrace
        for prefix in _fstring_prefixes
    },
    **{
        f"{prefix}'''": single3prog
        for prefix in _strprefixes
    },
    **{
    ...
    **{
        f"{prefix}'''": single3prog_plus_lbrace
        for prefix in _fstring_prefixes
    },
    **{
...
def tokenize(
    ...
    tokeneater: TokenEater = printtoken,
) -> None:
    """
...
def untokenize(
    ...
    """
    ut = Untokenizer()
    return ut.untokenize(
        iterable
    )


def is_fstring_start(
    token: str,
) -> bool:
    return builtins.any(
        token.startswith(
...
def _split_fstring_start_and_middle(
    token: str,
) -> tuple[str, str]:
    for (
        prefix
    ...
    raise ValueError(
        f"Token {token!r} is not a valid f-string start"
    )


STATE_NOT_FSTRING: (
    Final
) = 0  # not in an f-string
STATE_MIDDLE: Final = (
    1  # in the string portion of an f-string (outside braces)
)
STATE_IN_BRACES: Final = (
    2  # between braces in an f-string
)
...
def generate_tokens(
    ...
    while (
        ...
        if (
            ...
            endprog = endprog_stack[
                -1
            ]
            endmatch = endprog.match(
                line
            )
            if endmatch:
                end = endmatch.end(
        ...
        while pos < max:
            ...
            if (
                ...
                match = fstring_middle_after_colon.match(
                    line,
                    pos,
                )
                if (
            ...
            if (
                ...
                if (
                    ...
                    endprog_stack.append(
                        endprog
                    )
                    parenlev_stack.append(
                        parenlev
                    )
                    parenlev = (
                        0
