...
class Converter(grammar.Grammar):
    ...
    def parse_graminit_h(self, filename):
        ...
        for line in f:
            ...
            if not mo and line.strip():
                print(
                    f"{filename}({lineno}): can't parse {line.strip()}"
                )
            else:
                (
                    symbol,
                ...
                assert (
                    symbol
                    not in self.symbol2number
                )
                assert (
                    number
                    not in self.number2symbol
                )
                self.symbol2number[symbol] = (
                    number
                )
                self.number2symbol[number] = (
                    symbol
                )
        return True
    ...
    def parse_graminit_c(self, filename):
        ...
        nlabels = int(mo.group(1))
        assert nlabels == len(self.labels), (
            lineno,
            line,
        )
        lineno, line = (
        ...
        assert start in self.number2symbol, (
            lineno,
            line,
        )
        self.start = start
