...
class TokenProxy:
    ...
    def eat(self, point: int) -> Any:
        eaten_tokens = self._release_ranges[
            -1
        ].tokens
...
class Driver:
    ...
    def parse_tokens(
        ...
        type = value = start = end = line_text = (
            None
        )
        prefix = ""
...
def load_grammar(
    ...
    if logger is None:
        logger = logging.getLogger(__name__)
    gp = (
        _generate_pickle_name(gt)
        if gp is None
        else gp
    )
    if force or not _newer(gp, gt):
        g: grammar.Grammar = (
            pgen.generate_grammar(gt)
        )
        if save:
            try:
                g.dump(gp)
...
def load_packaged_grammar(
    ...
    pickled_name = _generate_pickle_name(
        os.path.basename(grammar_source),
        cache_dir,
    )
