...
from pip._internal.network.cache import (
    is_from_cache,
)
...
def _prepare_download(
    ...
    total_length = _get_http_response_size(
        resp
    )
    ...
    logged_url = redact_auth_from_url(
        url
    )
...
def parse_content_disposition(
    ...
    if filename:
        ...
        filename = sanitize_content_filename(
            str(filename)
        )
    return (
        filename
...
class Downloader:
    ...
    def __call__(
        ...
        filename = _get_http_response_filename(
            resp, link
        )
        filepath = (
            os.path.join(
