...
class KeyRingPythonProvider(
    ...
    def get_auth_info(
        ...
        if hasattr(
            ...
            logger.debug(
                "Getting credentials from keyring for %s",
                url,
            )
            cred = self.keyring.get_credential(
                url,
                username,
            )
            if (
        ...
        if (
            ...
            logger.debug(
                "Getting password from keyring for %s",
                url,
            )
...
def get_keyring_provider(
    ...
    if provider in [
        ...
        if (
            ...
            for (
                path
            ) in PATH_as_shutil_which_determines_it().split(
                os.pathsep
            ):
                p = Path(
                    path
        ...
        if cli:
            logger.verbose(
                "Keyring provider set: subprocess with executable %s",
                cli,
            )
...
class MultiDomainBasicAuth(
    ...
    def __init__(
        ...
        self._credentials_to_save: Optional[
            Credentials
        ] = None
    ...
    def _get_keyring_auth(
        ...
        try:
            return self.keyring_provider.get_auth_info(
                url,
                username,
            )
        except (
    ...
    def _get_index_url(
        ...
        for (
            ...
            parsed_index = urllib.parse.urlsplit(
                remove_auth_from_url(
                    index
                )
            )
    ...
    def _get_new_credentials(
        ...
        *,
        ...
        if (
            ...
            logger.debug(
                "Found credentials in url for %s",
                netloc,
            )
        ...
        if index_url:
            ...
            index_info = split_auth_netloc_from_url(
                index_url
            )
            if index_info:
                (
        ...
        if (
            ...
            if (
                ...
                logger.debug(
                    "Found credentials in index url for %s",
                    netloc,
                )
        ...
        if allow_netrc:
            ...
            if netrc_auth:
                logger.debug(
                    "Found credentials in netrc for %s",
                    netloc,
                )
                return netrc_auth
        ...
        if allow_keyring:
            ...
            if kr_auth:
                logger.debug(
                    "Found credentials in keyring for %s",
                    netloc,
                )
                return (
                    kr_auth
    ...
    def handle_401(
        ...
        **kwargs: Any,
        ...
        new_resp = resp.connection.send(
            req, **kwargs
        )
        new_resp.history.append(
            resp
        )
