...
from pip._internal.distributions.base import (
...
class SourceDistribution(
    ...
    def build_tracker_id(
        ...
        return (
            self.req.link.url_without_fragment
        )
    ...
    def prepare_distribution_metadata(
        ...
        if should_check_deps:
            ...
            (
                conflicting,
                missing,
            ) = self.req.build_env.check_requirements(
                pyproject_requires
            )
            if conflicting:
    ...
    def _get_build_requires_wheel(
        ...
        with self.req.build_env:
            ...
            backend = (
                self.req.pep517_backend
            )
            assert (
                backend
                is not None
            )
            with backend.subprocess_runner(
                runner
            ):
                return (
    ...
    def _install_build_reqs(
        ...
        if (
            ...
            build_reqs = (
                self._get_build_requires_editable()
            )
        else:
            build_reqs = (
                self._get_build_requires_wheel()
            )
        (
            conflicting,
            missing,
