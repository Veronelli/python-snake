...
class InstallCommand(
    ...
    def add_options(
        ...
        cmdoptions.add_target_python_options(
            self.cmd_opts
        )
        ...
        self.cmd_opts.add_option(
            cmdoptions.ignore_requires_python()
        )
        self.cmd_opts.add_option(
        ...
        self.cmd_opts.add_option(
            cmdoptions.progress_bar()
        )
        self.cmd_opts.add_option(
            cmdoptions.root_user_action()
        )
        ...
        self.parser.insert_option_group(
            0, index_opts
        )
        self.parser.insert_option_group(
            0,
    ...
    def run(
        ...
        installing_into_current_environment = (
            not (
                options.dry_run
                and options.json_report_file
            )
            and options.root_path
            is None
            and options.target_dir
            is None
            and options.prefix_path
            is None
        )
        if (
            installing_into_current_environment
        ...
        cmdoptions.check_dist_restriction(
            options,
            check_target=True,
        )
        ...
        target_temp_dir: Optional[
            TempDirectory
        ] = None
        target_temp_dir_path: Optional[
            str
        ] = None
        if (
            options.target_dir
            ...
            options.target_dir = os.path.abspath(
                options.target_dir
            )
            if (
                # fmt: off
        ...
        target_python = make_target_python(
            options
        )
        ...
        build_tracker = self.enter_context(
            get_build_tracker()
        )
        ...
        try:
            ...
            try:
                pip_req = requirement_set.get_requirement(
                    "pip"
                )
            except (
                KeyError
            ...
            warn_script_location = (
                options.warn_script_location
            )
            if (
                options.target_dir
                or options.prefix_path
            ):
                warn_script_location = (
                    False
                )
            ...
            if (
                ...
                self._warn_about_conflicts(
                    conflicts,
                    resolver_variant=self.determine_resolver_variant(
                        options
                    ),
                )
            ...
            for (
                ...
                if (
                    ...
                    if any(
                        ...
                        for s in lib_dir_list[
                            :-1
                        ]
                    ):
                        continue
                target_item_dir = os.path.join(
                    target_dir,
                    item,
                )
                if os.path.exists(
    ...
    def _determine_conflicts(
        ...
        try:
            return check_install_conflicts(
                to_install
            )
        except Exception:
            logger.exception(
