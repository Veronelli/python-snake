...
def _force_correct_text_stream(
    ...
    if is_binary(
        ...
        possible_binary_reader = find_binary(
            text_stream
        )
...
def get_text_stdin(
    ...
    rv = _get_windows_console_stream(
        sys.stdin,
        encoding,
        errors,
    )
...
def should_strip_ansi(
    ...
    if color is None:
        ...
        return not isatty(
            stream
        ) and not _is_jupyter_kernel_output(
            stream
        )
    return not color
...
if (
    ...
    def auto_wrap_for_ansi(  # noqa: F811
        ...
        """
        try:
            cached = _ansi_stream_wrappers.get(
                stream
            )
        except Exception:
            cached = None
        ...
        ansi_wrapper = colorama.AnsiToWin32(
            stream,
            strip=strip,
        )
        rv = t.cast(
