...
class Option:
    def __init__(
        ...
        for opt in opts:
            ...
            if not prefix:
                raise ValueError(
                    f"Invalid start character for option ({opt})"
                )
            self.prefixes.add(
                prefix[0]
            )
...
class OptionParser:
    ...
    def parse_args(
        ...
        try:
            ...
            if (
                self.ctx
                is None
                or not self.ctx.resilient_parsing
            ):
                raise
        return (
            state.opts,
    ...
    def _match_long_opt(
        ...
        if (
            ...
            value = self._get_value_from_state(
                opt,
                option,
                state,
            )
    ...
    def _process_opts(
        ...
        try:
            ...
            if (
                not self.ignore_unknown_options
            ):
                raise
