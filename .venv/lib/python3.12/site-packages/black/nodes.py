...
token.tok_name[STANDALONE_COMMENT] = (
    "STANDALONE_COMMENT"
)
...
def whitespace(
    ...
    *,
    ...
    if not prev:
        ...
        if (
            not prevp
            or prevp.type in OPENING_BRACKETS
        ):
            return NO
        ...
        if prevp.type == token.EQUAL:
            ...
            and parent_type(prevp)
            == syms.star_expr
            and (
                parent_type(prevp.parent)
                == syms.subscriptlist
                or (
                    Preview.pep646_typed_star_arg_type_var_tuple
                    in mode
    ...
    if p.type in {
        ...
        if not prev:
            ...
            if (
                not prevp
                or prevp.type == token.LPAR
            ):
                return NO

        elif (
            prev.type
            in {token.EQUAL} | VARARGS_SPECIALS
        ):
            return NO
        ...
        if (
            not prevp
            or prevp.type == token.AT
            or prevp.type == token.DOT
        ):
            return NO
        ...
        if not prev:
            ...
            ), "subscripts are always parented"
            if (
                p.parent.type
                == syms.subscriptlist
            ):
...
def is_one_tuple(
    ...
    if node.type == syms.atom:
        ...
        if (
            gexp is None
            or gexp.type != syms.testlist_gexp
        ):
            return False

        return (
            len(gexp.children) == 2
            and gexp.children[1].type
            == token.COMMA
        )
...
def is_tuple_containing_walrus(
    node: LN,
) -> bool:
...
def is_one_sequence_between(
    ...
    for (
        ...
        raise LookupError(
            "Opening paren not found in `leaves`"
        )
...
def is_walrus_assignment(
    ...
    return (
        inner is not None
        and inner.type == syms.namedexpr_test
    )


def is_simple_decorator_trailer(
    node: LN,
    last: bool = False,
) -> bool:
...
def is_simple_decorator_expression(
    node: LN,
) -> bool:
...
def is_vararg(
    ...
    """
    if (
        leaf.type not in VARARGS_SPECIALS
        or not leaf.parent
    ):
        return False
...
def fstring_to_string(
    node: Node,
) -> Leaf:
    """Converts an fstring node back to a string node."""
    string_without_prefix = str(node)[
        len(node.prefix) :
    ]
    string_leaf = Leaf(
        token.STRING,
...
def is_multiline_string(
    ...
    if isinstance(node, Node) and is_fstring(
        node
    ):
    ...
    return (
        has_triple_quotes(leaf.value)
        and "\n" in leaf.value
    )


def is_parent_function_or_class(
    node: Node,
) -> bool:
    assert node.type in {
        syms.suite,
...
def is_atom_with_invisible_parens(
    node: LN,
) -> bool:
...
def is_empty_par(
    ...
    return is_empty_lpar(leaf) or is_empty_rpar(
        leaf
    )
...
def is_with_or_async_with_stmt(
    ...
    return bool(
        ...
        and leaf.next_sibling.type
        == syms.with_stmt
    )


def is_async_stmt_or_funcdef(
    leaf: Leaf,
) -> bool:
...
def is_type_comment(
    ...
    return t in {
        token.COMMENT,
        STANDALONE_COMMENT,
    } and v.startswith("# type:")


def is_type_ignore_comment(
    leaf: Leaf,
) -> bool:
...
def is_type_ignore_comment_string(
    value: str,
) -> bool:
...
def unwrap_singleton_parenthesis(
    node: LN,
) -> Optional[LN]:
...
def get_annotation_type(
    leaf: Leaf,
) -> Literal["return", "param", None]:
    """Returns the type of annotation this leaf is part of, if any."""
    ancestor = leaf.parent
    while ancestor is not None:
        if (
            ancestor.prev_sibling
            and ancestor.prev_sibling.type
            == token.RARROW
        ):
            return "return"
        if (
            ancestor.parent
            and ancestor.parent.type == syms.tname
        ):
            return "param"
...
def furthest_ancestor_with_last_leaf(
    leaf: Leaf,
) -> LN:
