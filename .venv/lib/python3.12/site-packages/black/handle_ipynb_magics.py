...
def validate_cell(src: str, mode: Mode) -> None:
    ...
    """
    if any(
        transformed_magic in src
        for transformed_magic in TRANSFORMED_MAGICS
    ):
    ...
    if line.startswith("%%") and (
        ...
        not in PYTHON_CELL_MAGICS
        | mode.python_cell_magics
    ):
        raise NothingChanged


def remove_trailing_semicolon(
    src: str,
) -> tuple[str, bool]:
...
def mask_cell(
    ...
    transformed = (
        transformer_manager.transform_cell(src)
    )
    (
        transformed,
    ...
    transformed = (
        transformer_manager.transform_cell(
            transformed
        )
    )
    (
    ...
    if len(transformed.splitlines()) != len(
        src.splitlines()
    ):
...
def replace_cell_magics(
    ...
    return (
        f"{mask}\n{cell_magic_finder.cell_magic.body}",
        replacements,
    )
...
def replace_magics(
    ...
    for (
        ...
        if i in magic_finder.magics:
            offsets_and_magics = (
                magic_finder.magics[i]
            )
            if (
                len(offsets_and_magics) != 1
            ):  # pragma: nocover
                raise AssertionError(
