...
class ProtoComment:
    ...
    * `newlines` and `consumed` fields are kept separate from the `value`. This
    ...
    type: (
        int  # token.COMMENT or STANDALONE_COMMENT
    )
...
def list_comments(
    ...
    for (
        ...
        consumed += (
            len(full_line) + 1
        )  # adding the length of the split '\n'
        match = re.match(
            r"^(\s*)(\S.*|)$",
            full_line,
        )
...
def normalize_trailing_prefix(
    ...
    if "\\" not in remainder:
        ...
        leaf.prefix = make_simple_prefix(
            nl_count,
            form_feed,
        )
        return
...
def make_comment(
    ...
    if (
        ...
        content = (
            " " + content[1:]
        )  # Replace NBSP by a simple space
    if (
        content
        and content[0] not in COMMENT_EXCEPTIONS
    ):
        content = " " + content
    return "#" + content
...
def normalize_fmt_off(
    ...
    try_again = True
    while try_again:
        try_again = convert_one_fmt_off_pair(
            node,
            mode,
            lines,
        )
...
def convert_one_fmt_off_pair(
    ...
    for leaf in node.leaves():
        ...
        for comment in list_comments(
            ...
            if (
                not is_fmt_off and not is_fmt_skip
            ) or (
            ...
            hidden_value = "".join(
                str(n) for n in ignored_nodes
            )
            comment_lineno = (
                leaf.lineno - comment.newlines
            )
            if comment.value in FMT_OFF:
                fmt_off_prefix = ""
                if len(lines) > 0 and not any(
                    ...
                    fmt_off_prefix = prefix.split(
                        comment.value
                    )[0]
                    if "\n" in fmt_off_prefix:
                        fmt_off_prefix = (
                            fmt_off_prefix.split(
                                "\n"
                            )[-1]
                        )
                standalone_comment_prefix += (
            ...
            if is_fmt_skip:
                hidden_value += (
                    ...
                    if Preview.no_normalize_fmt_skip_whitespace
                    in mode
                    else "  "
                ) + comment.value
            ...
            parent.insert_child(
                ...
                Leaf(
                    ...
                    fmt_pass_converted_first_leaf=first_leaf_of(
                        first
                    ),
                ),
            )
...
def generate_ignored_nodes(
    ...
    while (
        ...
        and container.type != token.ENDMARKER
    ):
        ...
        if children_contains_fmt_on(container):
            for (
                index,
                child,
            ) in enumerate(container.children):
                if isinstance(
                    child,
                    Leaf,
                ) and is_fmt_on(child):
...
def _generate_ignored_nodes_from_fmt_skip(
    ...
    if (
        not comments
        or comment.value != comments[0].value
    ):
        return
    if prev_sibling is not None:
        ...
        while (
            ...
            and parent_sibling.type != syms.suite
        ):
        ...
        if (
            ...
            is not None
            and grandparent.prev_sibling.type
            == token.ASYNC
        ):
            ignored_nodes.insert(
                0,
                grandparent.prev_sibling,
            )
...
def _contains_fmt_skip_comment(
    comment_line: str,
    mode: Mode,
) -> bool:
    """
    ...
    semantic_comment_blocks = [
        ...
        *[
            ...
            for comment in comment_line.split(
                _COMMENT_PREFIX
            )[1:]
        ],
        *[
            _COMMENT_PREFIX + comment.strip()
            for comment in comment_line.strip(
                _COMMENT_PREFIX
