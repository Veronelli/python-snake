...
class LineGenerator(Visitor[Line]):
    ...
    def line(
        ...
        if len(
            self.current_line.leaves
        ) == 1 and is_async_stmt_or_funcdef(
    ...
    def visit_simple_stmt(
        ...
        if (
            node.parent
            and node.parent.type in STATEMENT
        ):
            if is_parent_function_or_class(
                node
            ) and is_stub_body(node):
                yield from self.visit_default(
                    node
                )
            else:
                yield from self.line(+1)
            ...
            if node.parent and is_stub_suite(
                node.parent
            ):
                node.prefix = ""
                yield from self.visit_default(
    ...
    def visit_STANDALONE_COMMENT(
        ...
        if (
            not self.current_line.bracket_tracker.any_open_brackets()
        ):
            yield from self.line()
        yield from self.visit_default(leaf)
    ...
    def visit_STRING(
        ...
        if is_docstring(
            ...
            if quote_len == 3:
                ...
                if (
                    len(lines) > 1
                    and last_line_length
                    + quote_len
                    > self.mode.line_length
                    and len(indent) + quote_len
                    <= self.mode.line_length
        ...
        if (
            self.mode.string_normalization
            and leaf.type == token.STRING
        ):
            leaf.value = normalize_string_prefix(
                leaf.value
            )
    ...
    def __post_init__(
        ...
        if (
            Preview.remove_redundant_guard_parens
            in self.mode
        ):
            self.visit_guard = partial(
                v,
                keywords=Ø,
                parens={"if"},
            )
    ...
    if (
        ...
        and (
            is_line_short_enough(
                line,
                mode=mode,
                line_str=line_str_hugging_power_ops,
            )
        ...
        line.is_def
        and not should_split_funcdef_with_rhs(
            line, mode
        )
    ):
...
def should_split_funcdef_with_rhs(
    line: Line, mode: Mode
) -> bool:
...
def right_hand_split(
    ...
    """
    rhs_result = _first_right_hand_split(
        line, omit=omit
    )
...
def _first_right_hand_split(
    ...
    if (
        ...
        and tail_leaves[0].opening_bracket
        is head_leaves[-1]
    ):
        inner_body_leaves = list(body_leaves)
        hugged_opening_leaves: list[Leaf] = []
        hugged_closing_leaves: list[Leaf] = []
        is_unpacking = body_leaves[0].type in [
            token.STAR,
            token.DOUBLESTAR,
        ]
        unpacking_offset: int = (
            1 if is_unpacking else 0
        )
        while (
            len(inner_body_leaves)
            >= 2 + unpacking_offset
            and inner_body_leaves[-1].type
            in CLOSING_BRACKETS
            and inner_body_leaves[
                -1
            ].opening_bracket
            is inner_body_leaves[unpacking_offset]
        ):
            if unpacking_offset:
                hugged_opening_leaves.append(
                    inner_body_leaves.pop(0)
                )
                unpacking_offset = 0
            hugged_opening_leaves.append(
                inner_body_leaves.pop(0)
            )
            hugged_closing_leaves.insert(
                0,
                inner_body_leaves.pop(),
            )
        ...
        if (
            ...
            if (
                ...
                line_length = line.mode.line_length - sum(
                    len(str(leaf))
                    for leaf in hugged_opening_leaves
...
def _maybe_split_omitting_optional_parens(
    ...
    if (
        ...
        omit = {
            id(rhs.closing_bracket),
            *omit,
        }
        try:
            # The RHSResult Omitting Optional Parens.
            rhs_oop = _first_right_hand_split(
                line,
                omit=omit,
            )
            is_split_right_after_equal = (
                len(rhs.head.leaves) >= 2
                and rhs.head.leaves[-2].type
                == token.EQUAL
            )
            rhs_head_contains_brackets = any(
                leaf.type in BRACKETS
                for leaf in rhs.head.leaves[:-1]
            )
            ...
            if (
                ...
                or _prefer_split_rhs_oop_over_rhs(
                    rhs_oop,
                    rhs,
                    mode,
                )
            ...
            elif not can_be_split(
                rhs.body
            ) and not is_line_short_enough(
                rhs.body,
                mode=mode,
            ):
                raise CannotSplit(
...
def _prefer_split_rhs_oop_over_rhs(
    ...
    rhs_head_equal_count = [
        leaf.type for leaf in rhs.head.leaves
    ].count(token.EQUAL)
    ...
    return (
        ...
        or (
            ...
            any(
                leaf.type == token.EQUAL
                for leaf in rhs_oop.head.leaves
            )
...
def _ensure_trailing_comma(
    ...
    leaf_with_parent = next(
        (leaf for leaf in leaves if leaf.parent),
        None,
    )
    ...
    if (
        get_annotation_type(leaf_with_parent)
        == "return"
    ):
        return False
    ...
    if (
        ...
        and leaf_with_parent.parent.next_sibling.type
        == token.VBAR
    ):
        return False
    return True
...
def bracket_split_build_line(
    ...
    *,
    ...
    for leaf in leaves:
        ...
        for (
            comment_after
        ) in original.comments_after(leaf):
            result.append(
                comment_after,
                preformatted=True,
            )
...
def _get_last_non_comment_leaf(
    line: Line,
) -> Optional[int]:
...
def delimiter_split(
    ...
    if (
        ...
        and bt.delimiter_count_with_priority(
            delimiter_priority
        )
        == 1
    ):
    ...
    def append_comments(
        ...
        for comment_after in line.comments_after(
            leaf
        ):
            yield from append_to_line(
                comment_after
            )

    last_non_comment_leaf = (
        _get_last_non_comment_leaf(line)
    )
    for (
        leaf_idx,
        leaf,
    ) in enumerate(line.leaves):
        yield from append_to_line(leaf)

        previous_priority = (
            leaf_idx > 0
            and bt.delimiters.get(
        ...
        if (
            trailing_comma_safe
            and leaf.bracket_depth == lowest_depth
        ):
            trailing_comma_safe = (
        ...
        leaf_priority = bt.delimiters.get(
            id(leaf)
        )
        if leaf_priority == delimiter_priority:
            if (
                leaf_idx + 1 < len(line.leaves)
                and delimiter_priority
                not in MIGRATE_COMMENT_DELIMITERS
            ):
                yield from append_comments(
                    line.leaves[leaf_idx + 1]
                )
...
def standalone_comment_split(
    ...
    if not line.contains_standalone_comments():
        raise CannotSplit(
            "Line does not have any standalone comments"
        )
...
def normalize_invisible_parens(  # noqa: C901
    ...
    *,
    ...
    for (
        index,
        child,
    ) in enumerate(list(node.children)):
        # Fixes a bug where invisible parens are not properly stripped from
        # assignment statements that contain type annotations.
        if (
            isinstance(child, Node)
            and child.type == syms.annassign
        ):
            normalize_invisible_parens(
                child,
        ...
        if (
            ...
            and Preview.parens_for_long_if_clauses_in_case_block
            in mode
        ):
            normalize_invisible_parens(
                child,
...
def remove_await_parens(
    node: Node,
) -> None:
    if (
        node.children[0].type == token.AWAIT
        and len(node.children) > 1
    ):
        if (
            node.children[1].type == syms.atom
            and node.children[1].children[0].type
            == token.LPAR
        ):
            if maybe_make_parens_invisible_in_atom(
                node.children[1],
            ...
            opening_bracket = cast(
                Leaf,
                node.children[1].children[0],
            )
            closing_bracket = cast(
                Leaf,
                node.children[1].children[-1],
            )
            bracket_contents = node.children[
                1
            ].children[1]
            if isinstance(
                bracket_contents,
                ...
                bracket_contents.type
                != syms.power
                or bracket_contents.children[
                    0
                ].type
                == token.AWAIT
                or any(
...
def _maybe_wrap_cms_in_parens(
    ...
    if colon_index is not None:
        ...
        context_managers = node.children[
            1:colon_index
        ]
...
def maybe_make_parens_invisible_in_atom(
    ...
    if (
        ...
        or (
            ...
            and max_delimiter_priority_in_atom(
                node
            )
            >= COMMA_PRIORITY
        )
    ...
    if is_lpar_token(first) and is_rpar_token(
        last
    ):
        ...
        if (
            ...
            not is_type_ignore_comment_string(
                middle.prefix.strip()
            )
        ):
            first.value = ""
...
def should_split_line(
    ...
    return max_priority == COMMA_PRIORITY and (
        ...
        or opening_bracket.parent.type
        in {
            syms.atom,
            syms.import_from,
        }
...
def generate_trailers_to_omit(
    ...
    for (
        ...
        has_inline_comment = leaf_length > len(
            leaf.value
        ) + len(leaf.prefix)
        if (
            leaf.type == STANDALONE_COMMENT
            or has_inline_comment
        ):
            break
        ...
        if opening_bracket:
            ...
            if (
                prev
                and prev.type in OPENING_BRACKETS
            ):
