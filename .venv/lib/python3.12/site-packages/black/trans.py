...
def hug_power_op(
    ...
    for (
        ...
        raise CannotTransform(
            "No doublestar token was found in the line."
        )

    def is_simple_lookup(
        index: int,
        kind: Literal[
            1, -1
        ],
        ...
        if (
            Preview.is_simple_lookup_for_doublestar_expression
            not in mode
        ):
            return original_is_simple_lookup_func(
                line,
                index,
                kind,
            )
...
class StringTransformer(
    ...
    def __call__(
        ...
        if not any(
            ...
            raise CannotTransform(
                "There are no strings in this line."
            )
...
class CustomSplitMapMixin:
    ...
    def pop_custom_splits(
        ...
        custom_splits = self._CUSTOM_SPLIT_MAP[
            key
        ]
        del self._CUSTOM_SPLIT_MAP[
            key
        ]
...
class StringMerger(
    ...
    def do_match(
        ...
        is_valid_index = is_valid_index_factory(
            LL
        )
        ...
        while (
            ...
            if (
                ...
                if (
                    not contains_comment
                    and not is_part_of_annotation(
                        leaf
                    )
                ):
    ...
    def do_transform(
        ...
        msg_result = self._merge_string_group(
            new_line,
            string_indices,
        )
        if isinstance(
    ...
    def _remove_backslash_line_continuation_chars(
        ...
        for (
            ...
            if (
                ...
                and not has_triple_quotes(
                    string_leaf.value
                )
            ):
                indices_to_transform.append(
                    string_idx
                )

        if (
            not indices_to_transform
        ):
            return TErr(
        ...
        for (
            string_idx
        ) in indices_to_transform:
            new_string_leaf = new_line.leaves[
                string_idx
            ]
    ...
    def _merge_string_group(
        ...
        if (
            not merged_string_idx_dict
        ):
            return TErr(
                "No string group is merged"
            )
        ...
        for (
            ...
            if (
                ...
                for comment_leaf in line.comments_after(
                    LL[i]
                ):
                    new_line.append(
                        comment_leaf,
    ...
    def _validate_msg(
        ...
        for (
            ...
            if (
                ...
                return TErr(
                    "StringMerger does NOT merge raw strings."
                )
...
class StringParenStripper(
    ...
    def do_match(
        ...
        return TErr(
            "This line has no strings wrapped in parens."
        )
        ...
        for (
            ...
            for leaf in (
                ...
                if line.comments_after(
                    ...
                    break
            if should_transform:
                string_and_rpar_indices.extend(
                    (
                        string_idx,
                        rpar_idx,
                    )
    ...
    def _transform_to_new_line(
        ...
        for idx in sorted(
            ...
            if (
                ...
                # replace comments
                old_comments = new_line.comments.pop(
                    id(
                        LL[
                            idx
                        ]
...
class BaseStringSplitter(
    ...
    def do_match(
        ...
        match_result = self.do_splitter_match(
            line
        )
        if isinstance(
            match_result,
    ...
    def _validate(
        ...
        max_string_length = self._get_max_string_length(
            line,
            string_idx,
        )
        if (
        ...
        if not string_leaf.parent or [
            L.type
            for L in string_leaf.parent.children
        ] == [
            token.STRING,
            token.NEWLINE,
        ]:
        ...
        if has_triple_quotes(
            ...
            return TErr(
                "We cannot split multiline strings."
            )

        return Ok(None)

    def _get_max_string_length(
        self,
        line: Line,
        string_idx: int,
    ) -> int:
    ...
    def _prefer_paren_wrap_match(
        ...
        if (
            ...
            if (
                not prev_sibling
                or prev_sibling.type
                == token.COMMA
            ) and (
                not next_sibling
                or next_sibling.type
                == token.COMMA
            ):
                return 0
...
class StringSplitter(
    ...
    def do_splitter_match(
        ...
        if (
            ...
            return TErr(
                "Line needs to be wrapped in parens first."
            )
        ...
        if is_valid_index(
            ...
            return TErr(
                "This line does not end with a string."
            )
    ...
    def do_transform(
        ...
        insert_str_child = insert_str_child_factory(
            LL[string_idx]
        )
        ...
        string_op_leaves = self._get_string_operator_leaves(
            LL
        )
        string_op_leaves_length = (
            sum(
                ...
                for prefix_leaf in string_op_leaves
            )
            + 1
            if string_op_leaves
            else 0
        ...
        def maybe_append_string_operators(
            new_line: Line,
        ) -> None:
            """
            Side Effects:
        ...
        custom_splits = self.pop_custom_splits(
            LL[
                string_idx
            ].value
        )
        ...
        while (
            ...
            if drop_pointless_f_prefix:
                next_value = self._normalize_f_string(
                    next_value,
                    prefix,
                )
        ...
        if drop_pointless_f_prefix:
            rest_value = self._normalize_f_string(
                rest_value,
                prefix,
            )
    ...
    def _iter_nameescape_slices(
        ...
        for idx, c in it:
            ...
            begin = (
                idx - 1
            )  # the position of backslash before \N{...}
            for (
                idx,
                c,
            ) in it:
    ...
    def _iter_fexpr_slices(
        ...
        if (
            "f"
            not in get_string_prefix(
                string
            ).lower()
        ):
            return
        yield from iter_fexpr_spans(
            string
        )
    ...
    def _get_illegal_split_indices(
        ...
        iterators = [
            self._iter_fexpr_slices(
                string
            ),
            self._iter_nameescape_slices(
                string
            ),
        ]
        for (
    ...
    def _get_break_idx(
        ...
        def breaks_unsplittable_expression(
            i: Index,
        ) -> bool:
            """
            Returns:
        ...
        def passes_all_checks(
            ...
            return (
                ...
                and is_not_escaped
                and is_big_enough
                and not breaks_unsplittable_expression(
                    i
                )
            )
    ...
    def _maybe_normalize_string_quotes(
        self, leaf: Leaf
    ) -> None:
        if (
            self.normalize_strings
...
class StringParenWrapper(
    ...
    def do_splitter_match(
        ...
        string_idx = (
            ...
            or self._assert_match(
                LL
            )
            or self._assign_match(
                LL
            )
            or self._dict_or_lambda_match(
                LL
            )
            or self._prefer_paren_wrap_match(
                LL
            )
        )
        ...
        return TErr(
            "This line does not contain any non-atomic strings."
        )
    ...
    def do_transform(
        ...
        for (
            leaf
        ) in leaves_to_steal_comments_from:
            for comment_leaf in line.comments_after(
                leaf
            ):
                first_line.append(
