    ...
    for directory in (
        ...
        *common_base.parents,
        ...
        if (
            ...
            pyproject_toml = _load_toml(
                directory / "pyproject.toml"
            )
            if "black" in pyproject_toml.get(
                "tool",
                {},
            ):
                return (
...
def find_pyproject_toml(
    ...
    path_pyproject_toml = (
        path_project_root / "pyproject.toml"
    )
    if path_pyproject_toml.is_file():
        return str(path_pyproject_toml)

    try:
        path_user_pyproject_toml = (
        ...
        err(
            f"Ignoring user configuration directory due to {e!r}"
        )
        return None
...
def parse_pyproject_toml(
    ...
    config: dict[str, Any] = pyproject_toml.get(
        "tool", {}
    ).get("black", {})
    config = {
...
def infer_target_version(
    ...
    """
    project_metadata = pyproject_toml.get(
        "project", {}
    )
    requires_python = project_metadata.get(
        "requires-python",
        None,
    )
    if requires_python is not None:
        try:
            return parse_req_python_version(
                requires_python
            )
        except InvalidVersion:
            pass
        try:
            return parse_req_python_specifier(
                requires_python
            )
        except (
            InvalidSpecifier,
...
def parse_req_python_specifier(
    ...
    """
    specifier_set = strip_specifier_set(
    ...
    target_version_map = {
        f"3.{v.value}": v for v in TargetVersion
    }
    ...
    if compatible_versions:
        return [
            target_version_map[v]
            for v in compatible_versions
        ]
    return None


def strip_specifier_set(
    ...
    return SpecifierSet(
        ",".join(str(s) for s in specifiers)
    )
...
def path_is_excluded(
    ...
    match = (
        pattern.search(normalized_path)
        if pattern
        else None
    )
...
def gen_python_files(
    ...
    *,
    ...
    for child in paths:
        ...
        root_relative_path = child.relative_to(
            root
        ).as_posix()
        ...
        if child.is_dir():
            ...
            if (
                ...
                continue
            include_match = (
                include.search(root_relative_path)
                if include
                else True
            )
            if include_match:
