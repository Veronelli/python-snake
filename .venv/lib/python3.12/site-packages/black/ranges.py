...
def adjusted_lines(
    ...
    for (
        ...
        if start_mapping_index >= len(
            lines_mappings
        ) or end_mapping_index >= len(
            lines_mappings
        ):
            # Protect against invalid inputs.
            continue
        start_mapping = lines_mappings[
            start_mapping_index
        ]
        end_mapping = lines_mappings[
            end_mapping_index
        ]
        ...
        if end_mapping.is_changed_block:
            ...
            new_end = (
                ...
                - end_mapping.original_start
                + end_mapping.modified_start
            )
        new_range = (
            new_start,
            new_end,
...
def convert_unchanged_lines(
    ...
    visitor = _TopLevelStatementsVisitor(
        lines_set
    )
    _ = list(
        visitor.visit(src_node)
    )  # Consume all results.
...
def _contains_standalone_comment(
    node: LN,
) -> bool:
    if isinstance(node, Leaf):
        ...
        for child in node.children:
            if _contains_standalone_comment(
                child
            ):
                return True
        return False
...
class _TopLevelStatementsVisitor(Visitor[None]):
    ...
    def visit_suite(
        ...
        if semantic_parent is not None:
            if (
                semantic_parent.prev_sibling
                is not None
                and semantic_parent.prev_sibling.type
                == ASYNC
            ):
                semantic_parent = (
...
def _convert_unchanged_line_by_line(
    node: Node,
    lines_set: set[int],
) -> None:
    ...
    for leaf in node.leaves():
        ...
        if (
            ...
            leaf.parent
            and leaf.parent.type == syms.suite
        ):
            # The `suite` node is defined as:
...
def _get_line_range(
    node_or_nodes: Union[LN, list[LN]]
) -> set[int]:
...
def _calculate_lines_mappings(
    ...
    for (
        ...
        if i == 0:
            ...
            previous_block = matching_blocks[
                i - 1
            ]
            lines_mappings.append(
                _LinesMapping(
                    original_start=previous_block.a
                    + previous_block.size
                    + 1,
                    original_end=block.a,
                    modified_start=previous_block.b
                    + previous_block.size
                    + 1,
                    modified_end=block.b,
                    is_changed_block=True,
...
def _find_lines_mapping_index(
    ...
    while index < len(lines_mappings):
        ...
        if (
            mapping.original_start
            <= original_line
            <= mapping.original_end
        ):
            return index
        index += 1
    return index
