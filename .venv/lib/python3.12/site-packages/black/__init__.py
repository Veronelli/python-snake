...
COMPILED = Path(__file__).suffix in (
    ".pyd",
    ".so",
)
...
def read_pyproject_toml(
    ...
    if exclude is not None and not isinstance(
        exclude, str
    ):
        raise click.BadOptionUsage(
            "exclude",
...
def spellcheck_pyproject_toml_keys(
    ...
    available_config_options = {
        param.name for param in ctx.command.params
    }
...
def target_version_option_callback(
    ...
    """
    return [
        TargetVersion[val.upper()] for val in v
    ]
...
def validate_regex(
    ...
    try:
        return (
            re_compile_maybe_verbose(value)
...
@click.option(
    ...
    "--target-version",
    type=click.Choice(
        [v.name.lower() for v in TargetVersion]
    ),
...
def main(  # noqa: C901
    ...
    if enable_unstable_feature and not (
        preview or unstable
    ):
        out(
            main.get_usage(ctx)
    ...
    if verbose:
        ...
        if config:
            ...
            if config == user_level_config:
                ...
                out(
                    f"Using configuration in '{config}'.",
                    fg="blue",
                )
            if ctx.default_map:
                for (
    ...
    if ipynb and pyi:
        err(
            "Cannot pass both `pyi` and `ipynb` flags!"
        )
        ctx.exit(1)
    ...
    if code is not None:
        ...
        assert (
            root is not None
        )  # root is only None if code is not None
        try:
            sources = get_sources(
                root=root,
                src=src,
    ...
    *,
    ...
    for s in src:
        ...
        if is_stdin or path.is_file():
            if resolves_outside_root_or_cannot_stat(
                path,
                root,
                report,
            ):
                if verbose:
                    out(
                        f'Skipping invalid source: "{path}"',
                        fg="red",
                    )
                continue
            ...
            if verbose:
                out(
                    f'Found input source directory: "{path}"',
                    fg="blue",
                )
...
def format_file_in_place(
    ...
    *,
    ...
    src_contents = (
        header.decode(encoding) + src_contents
    )
    dst_contents = (
        header.decode(encoding) + dst_contents
    )
    ...
    *,
    ...
    if content is None:
        (
            ...
            encoding,
            newline,
        ) = decode_bytes(sys.stdin.buffer.read())
    else:
        (
            src,
            encoding,
...
def format_file_contents(
    ...
    *,
    ...
    """
    if mode.is_ipynb:
        dst_contents = format_ipynb_string(
            src_contents,
            fast=fast,
            mode=mode,
        )
...
def format_cell(
    ...
    *,
    ...
    dst_without_trailing_semicolon = unmask_cell(
        masked_dst,
        replacements,
    )
    dst = put_trailing_semicolon_back(
        dst_without_trailing_semicolon,
        has_trailing_semicolon,
    )
    dst = dst.rstrip("\n")
    if dst == src:
...
def format_str(
    ...
    *,
    ...
    if lines:
        ...
        if not lines:
            return (
                src_contents  # Nothing to format
            )
...
def _format_str_once(
    ...
    *,
    ...
    if mode.target_versions:
        ...
        future_imports = get_future_imports(
            src_node
        )
    ...
    for current_line in line_generator.visit(
        src_node
    ):
        block = elt.maybe_empty_lines(
            current_line
        )
    ...
    *,
    ...
    for n in node.pre_order():
        if n.type == token.FSTRING_START:
            ...
            if len(
                n.children
            ) > 1 and not is_simple_decorator_expression(
                n.children[1]
            ):
                features.add(
            ...
            if (
                len(atom_children) == 3
                and atom_children[0].type
                == token.LPAR
                and _contains_asexpr(
                    atom_children[1]
                )
                and atom_children[2].type
                == token.RPAR
            ):
                features.add(
                    Feature.PARENTHESIZED_CONTEXT_MANAGERS
...
def _contains_asexpr(
    ...
    if node.type == syms.asexpr_test:
        ...
        if (
            ...
            return _contains_asexpr(
                node.children[1]
            )
    elif node.type == syms.testlist_gexp:
        return any(
            _contains_asexpr(child)
    ...
    def get_imports_from_children(
        ...
        for child in children:
            if isinstance(child, Leaf):
                ...
                child.type == syms.import_as_name
            ):
                ...
                child.type == syms.import_as_names
            ):
                yield from get_imports_from_children(
                    child.children
                )
...
def assert_equivalent(src: str, dst: str) -> None:
    ...
    src_ast_str = "\n".join(
        stringify_ast(src_ast)
    )
    dst_ast_str = "\n".join(
        stringify_ast(dst_ast)
    )
