...
class BracketTracker:
    ...
    depth: int = 0
    bracket_match: dict[
        tuple[Depth, NodeType],
        Leaf,
    ] = field(default_factory=dict)
    delimiters: dict[LeafID, Priority] = field(
        default_factory=dict
    )
    previous: Optional[Leaf] = None
    _for_loop_depths: list[int] = field(
        default_factory=list
    )
    _lambda_argument_depths: list[int] = field(
        default_factory=list
    )
    invisible: list[Leaf] = field(
        default_factory=list
    )
    ...
    def mark(self, leaf: Leaf) -> None:
        ...
        if self.depth == 0:
            delim = is_split_before_delimiter(
                leaf,
                self.previous,
            )
            if (
                ...
                delim = is_split_after_delimiter(
                    leaf
                )
                if delim:
                    self.delimiters[id(leaf)] = (
                        delim
                    )
    ...
    def any_open_for_or_lambda(
        ...
        return bool(
            ...
            or self._lambda_argument_depths
        )
    ...
    def delimiter_count_with_priority(
        self,
        priority: Priority = 0,
    ) -> int:
        ...
        priority = (
            priority
            or self.max_delimiter_priority()
        )
        return sum(
            1
    ...
    def maybe_increment_lambda_arguments(
        self, leaf: Leaf
    ) -> bool:
    ...
    def maybe_decrement_after_lambda_arguments(
        ...
        if (
            ...
            and self._lambda_argument_depths[-1]
            == self.depth
...
def is_split_before_delimiter(
    ...
    if (
        ...
        and (
            previous is None
            or previous.type in CLOSING_BRACKETS
        )
    ):
        return DOT_PRIORITY
...
def max_delimiter_priority_in_atom(
    node: LN,
) -> Priority:
...
def get_leaves_inside_matching_brackets(
