...
from black._width_table import (
    WIDTH_TABLE,
)
...
def has_triple_quotes(
    ...
    """
    raw_string = string.lstrip(
        STRING_PREFIX_CHARS
    )
    return raw_string[:3] in {'"""', "'''"}


def lines_with_leading_tabs_expanded(
    s: str,
) -> list[str]:
    """
    ...
    for line in s.splitlines():
        ...
        if (
            not stripped_line
            or stripped_line == line
        ):
            lines.append(line)
        else:
            prefix_length = len(line) - len(
                stripped_line
            )
            prefix = line[
                :prefix_length
...
def fix_docstring(
    ...
    if not docstring:
        return ""
    lines = lines_with_leading_tabs_expanded(
        docstring
    )
...
def get_string_prefix(
    ...
    prefix_idx = 0
    while (
        string[prefix_idx] in STRING_PREFIX_CHARS
    ):
...
def normalize_string_quotes(
    ...
    unescaped_new_quote = _cached_compile(
        rf"(([^\\]|^)(\\\\)*){new_quote}"
    )
    escaped_new_quote = _cached_compile(
        rf"([^\\]|^)\\((?:\\\\)*){new_quote}"
    )
...
def normalize_fstring_quotes(
    ...
    if new_escape_count > orig_escape_count:
        return (
            middles,
            quote,
        )  # Do not introduce more escaping
...
def normalize_unicode_escape_sequences(
    leaf: Leaf,
) -> None:
