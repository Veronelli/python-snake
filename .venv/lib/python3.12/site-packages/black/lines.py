...
class Line:
    ...
    bracket_tracker: BracketTracker = field(
        default_factory=BracketTracker
    )
    ...
    def append(
        ...
        if (
            token.COLON == leaf.type
            and self.is_class_paren_empty
        ):
            del self.leaves[-2:]
        if self.leaves and not preformatted:
            ...
            leaf.prefix += whitespace(
                leaf,
                complex_subscript=self.is_complex_subscript(
                    leaf
                ),
                mode=self.mode,
            )
        if (
            ...
            if self.mode.magic_trailing_comma:
                if self.has_magic_trailing_comma(
                    leaf
                ):
                    self.magic_trailing_comma = (
                        leaf
                    )
            elif self.has_magic_trailing_comma(
                leaf
            ):
    ...
    def append_safe(
        ...
        if (
            ...
            if self.is_comment:
                raise ValueError(
                    "cannot append to standalone comments"
                )
    ...
    def is_decorator(
        ...
        return (
            bool(self)
            and self.leaves[0].type == token.AT
        )
    ...
    def is_import(
        ...
        return bool(self) and is_import(
            self.leaves[0]
        )

    @property
    def is_with_or_async_with_stmt(
        self,
    ) -> bool:
        """Is this a with_stmt line?"""
        return bool(
            self
        ) and is_with_or_async_with_stmt(
            self.leaves[0]
        )
    ...
    def is_stub_class(
        ...
        return self.is_class and self.leaves[
            -3:
        ] == [
            Leaf(
                token.DOT,
    ...
    def is_stub_def(
        ...
        return self.is_def and self.leaves[
            -4:
        ] == [
            Leaf(
                token.COLON,
    ...
    @property
    def _is_triple_quoted_string(
        self,
    ) -> bool:
        """Is the line a triple quoted string?"""
        if (
            not self
            or self.leaves[0].type != token.STRING
        ):
            return False
    ...
    def is_docstring(
        ...
        if (
            Preview.unify_docstring_detection
            not in self.mode
        ):
            return self._is_triple_quoted_string
        return bool(self) and is_docstring(
            self.leaves[0],
            self.mode,
        )

    @property
    def is_chained_assignment(
        self,
    ) -> bool:
        """Is the line a chained assignment"""
        return [
            leaf.type for leaf in self.leaves
        ].count(token.EQUAL) > 1
    ...
    def is_fmt_pass_converted(
        ...
        *,
        ...
        if (
            ...
            or leaf.fmt_pass_converted_first_leaf
            is None
        ):
            return False
        return (
            first_leaf_matches is None
            or first_leaf_matches(
    ...
    def contains_implicit_multiline_string_with_comments(
        self,
    ) -> bool:
    ...
    def contains_uncollapsable_type_comments(
        self,
    ) -> bool:
        ignored_ids = set()
        try:
        ...
        for (
            leaf_id,
            comments,
        ) in self.comments.items():
            for comment in comments:
                if is_type_comment(comment):
                    if comment_seen or (
                        not is_type_ignore_comment(
                            comment
                        )
                        and leaf_id
                        not in ignored_ids
    ...
    def contains_unsplittable_type_ignore(
        self,
    ) -> bool:
        if not self.leaves:
            return False
        ...
        if first_line == last_line:
            ...
            for node in self.leaves[-2:]:
                for comment in self.comments.get(
                    id(node),
                    [],
                ):
    ...
    def contains_multiline_strings(
        self,
    ) -> bool:
        return any(
            is_multiline_string(leaf)
            for leaf in self.leaves
        )
    ...
    def has_magic_trailing_comma(
        ...
        if closing.type == token.RSQB:
            if (
                ...
                assert (
                    closing.prev_sibling
                    is not None
                )
                assert (
                    closing.prev_sibling.type
                    == syms.subscriptlist
                )
                return False
    ...
    def is_complex_subscript(
        ...
        if isinstance(
            ...
            if (
                ...
                subscript_start = child_towards(
                    subscript_start,
                    leaf,
                )

        return subscript_start is not None and any(
            n.type in TEST_DESCENDANTS
            for n in subscript_start.pre_order()
        )
    ...
    def __str__(
        ...
        first = next(leaves)
        res = (
            f"{first.prefix}{indent}{first.value}"
        )
        for leaf in leaves:
            res += str(leaf)
        for (
...
class EmptyLineTracker:
    ...
    previous_defs: list[Line] = field(
        default_factory=list
    )
    semantic_leading_comment: Optional[
        LinesBlock
    ] = None
    ...
    def maybe_empty_lines(
        ...
        before, after = self._maybe_empty_lines(
            current_line
        )
        previous_after = (
            self.previous_block.after
        ...
        if (
            ...
            self.previous_block
            and self.previous_block.previous_block
            is None
            and len(
                self.previous_block.original_line.leaves
            )
            == 1
        ...
        if current_line.is_comment:
            if self.previous_line is None or (
                ...
                and (
                    not self.previous_line.is_comment
                    or before
                )
                and (
    ...
    def _maybe_empty_lines_for_class_or_def(  # noqa: C901
        ...
        if self.previous_line.is_decorator:
            if (
                self.mode.is_pyi
                and current_line.is_stub_class
            ):
        ...
        if (
            ...
            if self.mode.is_pyi:
                return 0, 0
            return (
                1 if user_had_newline else 0
            ), 0

        comment_to_add_newlines: Optional[
            LinesBlock
        ] = None
        if (
            self.previous_line.is_comment
            and self.previous_line.depth
            == current_line.depth
            and before == 0
        ):
        ...
        if comment_to_add_newlines is not None:
            previous_block = (
                comment_to_add_newlines.previous_block
            )
...
def append_leaves(
    ...
    for old_leaf in leaves:
        ...
        for (
            comment_leaf
        ) in old_line.comments_after(old_leaf):
            new_line.append(
                comment_leaf,
                preformatted=True,
            )
...
def is_line_short_enough(  # noqa: C901
    ...
    *,
    ...
    commas: list[int] = (
        []
    )  # tracks number of commas per depth level
    ...
    for (
        ...
        if max_level_to_update == math.inf:
            ...
            if leaf.bracket_depth + 1 > len(
                commas
            ):
                commas.append(0)
            elif leaf.bracket_depth + 1 < len(
                commas
            ):
            ...
            if (
                ...
                and multiline_string.bracket_depth
        ...
        if (
            ...
            if (
                ...
                or leaf.bracket_depth > 0
            ) and (
                i != len(line.leaves) - 1
                or leaf.prev_sibling
                not in ignore_ctxs
            ):
                commas[leaf.bracket_depth] += 1
        if max_level_to_update != math.inf:
            max_level_to_update = min(
                max_level_to_update,
                leaf.bracket_depth,
            )
...
def can_be_split(
    ...
    if (
        leaves[0].type == token.STRING
        and leaves[1].type == token.DOT
    ):
        call_count = 0
        dot_count = 0
        next = leaves[-1]
        for leaf in leaves[-2::-1]:
            if leaf.type in OPENING_BRACKETS:
                ...
                leaf.type not in CLOSING_BRACKETS
            ):
                return False
...
def can_omit_invisible_parens(
    ...
    for leaf in reversed(line.leaves):
        if (
            closing_bracket
            and leaf
            is closing_bracket.opening_bracket
        ):
            closing_bracket = None
        if (
            leaf.type == STANDALONE_COMMENT
            and not closing_bracket
        ):
            return False
        if (
            not closing_bracket
