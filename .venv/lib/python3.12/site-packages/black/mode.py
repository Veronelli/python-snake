...
def supports_feature(
    ...
    return all(
        feature in VERSION_TO_FEATURES[version]
...
@dataclass
class Mode:
    target_versions: set[TargetVersion] = field(
        default_factory=set
    )
    ...
    python_cell_magics: set[str] = field(
        default_factory=set
    )
    preview: bool = False
    unstable: bool = False
    enabled_features: set[Preview] = field(
        default_factory=set
    )
    ...
    def __contains__(
        ...
        return (
            self.preview
            and feature not in UNSTABLE_FEATURES
        )
    ...
    def get_cache_key(
        ...
        features_and_magics = (
            ...
            + "@"
            + ",".join(
                sorted(self.python_cell_magics)
            )
        )
        if (
            len(features_and_magics)
            > _MAX_CACHE_KEY_PART_LENGTH
        ):
