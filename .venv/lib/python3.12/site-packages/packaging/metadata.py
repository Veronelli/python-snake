...
def parse_email(
    ...
    for name in frozenset(
        ...
        raw_name = _EMAIL_TO_RAW_MAPPING.get(
            name
        )
        if (
            raw_name
...
class _Validator(
    ...
    def __set_name__(
        ...
        self.name = name
        self.raw_name = _RAW_TO_EMAIL_MAPPING[
            name
        ]
    ...
    def _process_metadata_version(
        ...
        if (
            value
            not in _VALID_METADATA_VERSIONS
        ):
    ...
    def _process_name(
        ...
        try:
            ...
            raise self._invalid_metadata(
                f"{value!r} is invalid for {{field}}",
                cause=exc,
            )
        else:
            return value
    ...
    def _process_version(
        ...
        try:
            return version_module.parse(
                value
            )
        except (
            version_module.InvalidVersion
        ) as exc:
            raise self._invalid_metadata(
                f"{value!r} is invalid for {{field}}",
                cause=exc,
...
class Metadata:
    ...
    def from_raw(
        ...
        *,
        ...
        if validate:
            ...
            try:
                metadata_version = (
                    ins.metadata_version
                )
                metadata_age = _VALID_METADATA_VERSIONS.index(
                    metadata_version
                )
            ...
            for (
                ...
                try:
                    if metadata_version:
                        ...
                        try:
                            field_metadata_version = cls.__dict__[
                                key
                            ].added
                        except KeyError:
                            exc = InvalidMetadata(
    ...
    *validate* parameter)"""
    ...
    description_content_type: _Validator[
        str | None
    ] = _Validator(
        added="2.1"
    )
