...
class Specifier(
    ...
    def __init__(
        ...
        """
        match = self._regex.search(
            spec
        )
        if not match:
            raise InvalidSpecifier(
                f"Invalid specifier: '{spec}'"
            )
    ...
    def _get_operator(
        ...
        operator_callable: CallableOperator = getattr(
            self,
            f"_compare_{self._operators[op]}",
        )
    ...
    def _compare_equal(
        ...
        if spec.endswith(
            ...
            split_prospective = _version_split(
            ...
            shortened_prospective = padded_prospective[
                : len(
                    split_spec
                )
            ]
    ...
    def contains(
        ...
        # "2.0" in Specifier(">=2")
        normalized_item = _coerce_version(
            item
        )
    ...
    def filter(
        ...
        for (
            ...
            parsed_version = _coerce_version(
                version
            )
...
class SpecifierSet(
    ...
    def __and__(
        ...
        if (
            self._prereleases
            is None
            and other._prereleases
            is not None
        ):
            specifier._prereleases = (
                other._prereleases
            )
        elif (
            self._prereleases
