...
def _normalize_extra_values(
    ...
    if isinstance(
        ...
        if (
            ...
            normalized_extra = canonicalize_name(
                rhs.value
            )
            rhs = Value(
                normalized_extra
...
_operators: dict[
    ...
    "in": lambda lhs, rhs: lhs
    in rhs,
    "not in": lambda lhs, rhs: lhs
    not in rhs,
    "<": operator.lt,
    "<=": operator.le,
    "==": operator.eq,
...
def format_full_version(
    info: sys._version_info,
) -> str:
    version = "{0.major}.{0.minor}.{0.micro}".format(
        info
    )
    kind = (
        info.releaselevel
